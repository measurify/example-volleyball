...
class _startPageState extends State<startPage> {
  ...
  void initState() {
    ...
    for (int i = 0; i < widget.globals.selectedItems.length; i++) {
  ...
  void _handleValueChange(
    ...
    if (characteristicId == "8e7c2dae-0002-4b0d-b516-f525649c49ca") {
      ...
      setState(() {
        ...
        //widget.globals.receivedIMU1JsonValues.add(jsonObj);
        //widget.globals.receivedIMUJsonValues[index]=widget.globals.receivedIMU1JsonValues;
        // Add data to chartDataIMUList for Device 1
  ...
  Future<void> sendData(String deviceId) async {
    ...
    if (response.statusCode == 200) {
      ...
      widget.globals.receivedIMUJsonValues[index] = [];
      widget.globals.savedValue = 0;
      //clear the list
              ...
              onPressed: () async {
                if (!widget.globals.option1 &&
                  ...
                  if (_selectedOption1 == "" || _selectedOption2 == "") {
                    ...
                    // Handle button press
                    print(widget.globals.selectedItems);
                    if (widget.globals.option1) {
                      if (widget.globals.isCollecting) {
                        ...
                        for (int i = 0;
                            i < widget.globals.selectedItems.length;
                            i++) {
                          await checkMeasureExist(
                              widget.globals.selectedItems[i]);
                        }
                        for (int i = 0;
                            i < widget.globals.selectedItems.length;
                            i++) {
              ...
              onPressed: () {
                ...
                if (widget.globals.option1) {
                  ...
                  for (int i = 0;
                      i < widget.globals.selectedItems.length;
                      i++) {
                    sendData(widget.globals.selectedItems[i]);
                  }
  ...
  Future<void> postMeasure(String deviceId) async {
    ...
    if (response.statusCode == 200) {
      ...
      // Convert each tag to a string and add it to the activityTags list
      if (widget.globals.measureName == '' ||
          widget.globals.measureName.isEmpty) {
